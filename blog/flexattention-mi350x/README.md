---
blogpost: true
blog_title: "Tuning PyTorch FlexAttention for AMD Instinct MI350X"
date: 14 August 2026
author: 'TODO: author list (add co-authors and reviewers before submission)'
thumbnail: '01-fp16-speedup.png'
tags: AI/ML, PyTorch, Performance, Optimization, Compiler, Hardware
category: Software tools & optimizations
target_audience: PyTorch users, GPU kernel developers, ML compiler engineers, and performance engineers
key_value_propositions: Learn how per-architecture kernel configuration defaults in PyTorch Inductor more than doubled fp16 FlexAttention throughput on AMD Instinct MI350X.
language: English
---
...
close depends almost entirely on the kernel configuration that PyTorch Inductor selects, and that
configuration comes from a per-architecture default table.
...
These defaults matter more on CDNA than they might appear to, because `num_warps` interacts with the
matrix cores. The MFMA instructions used by the FlexAttention template on CDNA4 produce a 32x32
output tile per wavefront, so the number of wavefronts determines how the query tile is divided and
whether that division matches the shape the matrix core computes natively. Because that interaction
depends on the matrix-core geometry of the specific part, the best value can differ from one
architecture to the next, which is why these defaults are now maintained per architecture.
...
Throughput in TFLOP/s; higher is better. The upstream review of this change swept roughly 20 shapes
with [attention-gym](https://github.com/meta-pytorch/attention-gym) and reported a geometric mean of
1.13x, which sits inside the 1.11x to 1.18x range above. If you want to check a broader set of shapes
than we show here, attention-gym is the quickest way to do it.
...
while bf16 used 4 for the same shapes. On gfx950, 4 turns out to be the better fit for both, and the
difference is substantial.

The reason comes down to MFMA geometry, and it is worth having in mind whenever you tune a CDNA
attention kernel. The instruction produces a 32-row output tile per wavefront, so with `BLOCK_M=128`,
four wavefronts tile the query block exactly, one MFMA tile each. Asking for eight divides that same
block more finely than the instruction's native shape. The same reasoning explains why gfx942, which
prefers `BLOCK_M=64`, pairs it with 8 warps instead. We did not isolate the mechanism beyond this, so
treat it as a rule of thumb rather than a proof — but if a CDNA kernel underperforms at a given tile
size, the relationship between `BLOCK_M`, the MFMA tile height, and `num_warps` is the first thing
worth checking.

Correcting the fp16 default to 4 ([pytorch#180720](https://github.com/pytorch/pytorch/pull/180720))
is a one-line change with an outsized effect:
...
to switch on. The part worth dwelling on is what did *not* change: the generated kernel is identical
before and after. All of that throughput was already sitting in the code Inductor emits, and a single
configuration value was leaving it on the floor. If you are tuning your own attention workload, that
is the encouraging read — the same lever is exposed to you through `kernel_options` and
...
corrected configuration landing on par with an optimized standalone Flash Attention kernel. That
comparison is worth following the PR for; we did not reproduce it here.
...
including the amount of LDS available per compute unit — 64 KB on MI300X against 160 KB on MI350X,
which PyTorch reports as 163,840 bytes of shared memory per block on the MI350X used here — and fp16
matrix throughput.
...
for targets without an entry. The resulting forward defaults, as
...
In practice this means MI300X now carries its own validated entry rather than inheriting values tuned
for a different part, and either architecture can be retuned later without regressing the other. If
you are bringing up a new target, this table is the place to add an entry. The gfx950 values are
unchanged by the split, so the numbers in this post are unaffected by it.
...
`tt.pointer_range=32` on HIP for tensor arguments whose storage provably fits within 2 GB. The
annotation is suppressed for kernels that use atomics, since buffer operations do not support them,
and it can be disabled with `TORCHINDUCTOR_EMIT_POINTER_RANGE_32=0` if it exposes a compiler issue.
...
Triton itself. These are enabling and correctness fixes rather than headline speedups, so the
throughput above does not rest on them, but they are the pieces that make the buffer-op path
available to any Triton kernel on AMD, not just this one:
...
Throughput is reported in TFLOP/s, computed as `2 * 2 * B * H * S * D` scaled by the fraction of the
score matrix that a causal mask leaves live. Because of that scaling, the causal and non-causal rows
...
machine, another process on the same device can cost several times the kernel's runtime on individual
points, which is easy to mistake for a real result.

Both are cheap to defend against. Every measurement here ramps clocks with several seconds of dense
matrix multiplication before timing, runs pinned to a device verified idle via `HIP_VISIBLE_DEVICES`,
and takes `triton.testing.do_bench` medians with a 200 ms warmup and 500 ms of repetitions, three
times per point and reduced by median. The reported figures are the median across three independent
process-level runs, where the median run-to-run spread is 0.5% and the worst case 4.9% on a single
bf16 point — enough to make the derived speedups stable to roughly +/- 0.02x.

Every timed configuration is also checked against an eager reference. The largest absolute deviations
were 2.0e-3 for fp16 and 1.6e-2 for bf16, which is what you should expect from a change in
accumulation order at these sequence lengths rather than a sign of a wrong result.

### Reproducing these results

The improved defaults require nothing beyond a recent PyTorch on ROCm; there is no new API and no
opt-in flag:
...
def bench(opts=None):
    ...
    ms = triton.testing.do_bench(lambda: fn(q, k, v), warmup=200, rep=500,
                                 return_mode="median")
    valid = (S * S + S) / 2  # causal
    return 2 * (2.0 * B * H * valid * D) / ms * 1e-9
...
Pin the run to an idle GPU so that neither clock ramping nor another tenant perturbs the result:
...
one change at a time. That approach also surfaces things an aggregate hides: with
`num_warps=8` still in place, pipelining alone is very slightly negative on the unmasked fp16
head-dimension-64 shape (0.98x), even though it helps every other shape measured, including the
causal case at the same head dimension. The gain only appears once the wavefront count is also right,
which is a good reason to tune those two together rather than one at a time.
